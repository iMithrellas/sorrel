...
func main() {
	...
	isValid, hasTimestamp, timestamp, err := validateYouTubeURL(text)
	if err != nil {
...
func validateYouTubeURL(link string) (isValid bool, hasTimestamp bool, timestamp string, err error) {
	// Parse the URL
	parsedURL, err := url.Parse(link)
	if err != nil {
		return false, false, "", fmt.Errorf("failed to parse URL: %w", err)
	}

	// Check if it’s a YouTube URL
	host := strings.ToLower(parsedURL.Host)
	if host != "www.youtube.com" && host != "youtube.com" && host != "youtu.be" {
		return false, false, "", fmt.Errorf("not a YouTube URL")
	}

	// Check for timestamp in query params
	query := parsedURL.Query()
	if ts, exists := query["t"]; exists {
		return true, true, ts[0], nil
	}
	...
	if matches := re.FindStringSubmatch(fragment); matches != nil {
		return true, true, matches[1], nil
	}
